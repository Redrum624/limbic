...
//   node sample-downloads.cjs badges --data .github/badges/downloads-data.json --out-dir .github/badges
//
...
// (the Vitrine pattern). Hot-linking img.shields.io/github/* for these depends
// on shields.io's own GitHub token pool, and when it drains the README shows
// "unable to select next github token from pool" error badges. The data both
// badges need is already in our sampled JSON, so we render them ourselves and
// shields can never break them again.
