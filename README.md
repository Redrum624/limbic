...
That last row is not a rounding claim. divsel computes every distance in `f32` with a fixed 16-accumulator reduction order, and its golden values were generated from that arithmetic; `src/internal/gist.ts` reproduces the same kernel with `Math.fround` rather than computing in `f64`, so the two agree bit-for-bit instead of merely within `1e-6`.
